...
test.describe("Lupen browser smoke", () => {
  ...
  test("multiplayer staging mode exposes staging UI without using real trade buttons", async ({ page }) => {
    ...
    await expect(page.locator("#lupenMultiplayerStagingFlowHint")).toContainText(/Trade for CR[\s\S]*Cargo Pod[\s\S]*Pulse Laser[\s\S]*Shield Booster[\s\S]*Erebus Patrol[\s\S]*Claim XP[\s\S]*Lupen Shard/i);
