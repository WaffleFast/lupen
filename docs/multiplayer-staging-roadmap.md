...
Store Phase 3 adds Cargo Pod equip/apply for `attachment:cargoPod` only. Local loadout mutation paths are fenced in `?mp=staging`; Cargo Pod equip routes through `stagingLoadout:equipAttachment`. It is disabled/dry-run by default and requires `STAGING_LOADOUT_WRITE_ENABLED=true`, `STAGING_LOADOUT_WRITE_DRY_RUN=false`, verified identity, loadout write scope/allowlist approval, `STAGING_LOADOUT_WRITE_ALLOWED_ITEMS` containing `attachment:cargoPod`, trusted `player_saves`, valid `currentShipId`, owned Cargo Pod count greater than zero, valid `shipLoadouts[currentShipId].attachments`, and an empty attachment slot. When every gate passes, the server decrements `ownedAttachments.cargoPod` and appends a standard level-1 Cargo Pod to `shipLoadouts[currentShipId].attachments`. The server derives Cargo Pod capacity with a narrow mirrored ship config and +25 Cargo Pod bonus. Credits, inventory, ships, weapons, guns, loot, bounties, PvP, player damage, trade cargo/totals, broad progression, and schema/RLS remain excluded.
