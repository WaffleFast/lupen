...
Staging weapon source of truth: the browser sends equipped weapon keys from `shipLoadouts[currentShipId].guns` through presence/combat intent metadata, but the server does not trust raw client damage values. The server resolves up to six known mounted guns, adds their server-owned damage into one capped volley, and uses the slowest gun cooldown. A Pulse Laser contributes `13` damage; two mounted Pulse Lasers therefore resolve `26` damage every `1,250 ms`. Missing or unknown-only weapon keys fall back to the existing safe staging damage (`5`) and report fallback diagnostics. Server-authoritative loadout derivation from trusted saved state is still a later phase.
...
Staging bounty wrapper status: [multiplayer-bounty-staging-design.md](multiplayer-bounty-staging-design.md) defines `Erebus Patrol Sweep`, a one-target Academy objective that teaches the full accept, scan, combat, return, claim, and repair loop. The other daily combat contracts retain their normal multi-target difficulty. In `?mp=staging`, the Bounty Board renders the server-owned contract state and refreshes after accept, bot destruction, and claim messages. Progress increments only for players who accepted the bounty and contributed to the destroyed bot.
...
- Next recommended UX/gameplay phase: polish player shield/stat display for equipped Shield Booster in staging. Do not combine that with broader equipment/loot design or PvP/player damage.
