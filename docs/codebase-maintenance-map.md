...
- Loadout slot rendering, inventory selection, tooltips, and equipment mutations
  have been extracted behind the same browser-global API. The former Hangar/Store
...
2. Continue modularizing `style.css` one screen at a time, starting with a stable
   ...
   formatting exercise.
3. Add focused coverage for the activity-feed visibility policy so large
   multiplayer scenarios do not need to own assertions about filtered presence
   and combat chatter.
4. Continue the server-authority migration in the order documented in
