...
When a server-owned staging bot is disabled, Colyseus builds a preview-only loot contract and attaches it to the existing `staging:reward_preview` event.
...
The loot preview does not write or mutate:
...
2. Add a gated inventory write for one material only.
