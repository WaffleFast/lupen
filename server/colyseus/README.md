...
- The future `player_saves` write adapter is implemented but disabled by default. It can build an XP/credits-only patch plan from known safe save paths, requires a stable idempotency key, marks duplicate reward applications, requires `STAGING_PROGRESSION_WRITE_ALLOWLIST` when progression writes are enabled, and defaults to `progression_writes_disabled`. Do not enable `ENABLE_STAGING_PROGRESSION_WRITES` outside controlled staging tests with explicit allow-listed accounts.
