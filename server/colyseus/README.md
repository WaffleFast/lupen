...
- Reward claim simulation also builds a future `player_saves` XP/credits patch plan through `src/services/playerSaveWriteService.js`. Progression writes are disabled by default with `skippedReason: progression_writes_disabled`. Each eligible plan includes a stable `idempotencyKey` derived from the verified player id plus `sourceEventId` or ledger id, and staging duplicate claims are marked with `duplicate_reward_application`. If `ENABLE_STAGING_PROGRESSION_WRITES=true` is set later, the adapter also requires the verified player id to appear in `STAGING_PROGRESSION_WRITE_ALLOWLIST`; missing lists return `staging_write_allowlist_missing`, and non-listed users return `player_not_in_staging_write_allowlist`. Even an allow-listed user still fails closed with `progression_write_adapter_not_implemented` until the final reviewed player save write path exists. This path is limited to XP and credits only and never writes loot, inventory, bounties, cargo, equipment, ships, saves, or other progression fields.
...
- The future `player_saves` write adapter is prepared but fail-closed. It can build an XP/credits-only patch plan from known safe save paths, requires a stable idempotency key, marks duplicate reward applications, requires `STAGING_PROGRESSION_WRITE_ALLOWLIST` when progression writes are enabled, and defaults to `progression_writes_disabled`. Enabled mode for an allow-listed verified user still returns `progression_write_adapter_not_implemented`; it must not be enabled for real progression writes yet.
...
- The player save write adapter builds XP/credits-only `player_saves` patch plans in mocked tests, returns `progression_writes_disabled` by default, exposes idempotency readiness, blocks duplicate reward applications, blocks missing or non-matching `STAGING_PROGRESSION_WRITE_ALLOWLIST` values, fails closed with `progression_write_adapter_not_implemented` for allow-listed verified users if progression writes are toggled on prematurely, and rejects missing or ambiguous XP/credits paths.
