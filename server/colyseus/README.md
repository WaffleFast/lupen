...
- Reward claim simulation also builds a future `player_saves` XP/credits patch plan through `src/services/playerSaveWriteService.js`. Progression writes are disabled by default with `skippedReason: progression_writes_disabled`; even if `ENABLE_STAGING_PROGRESSION_WRITES=true` is set later, the current adapter fails closed with `idempotency_not_ready` until durable duplicate protection is implemented. This path is limited to XP and credits only and never writes loot, inventory, bounties, cargo, equipment, ships, saves, or other progression fields.
...
- The player save write adapter builds XP/credits-only `player_saves` patch plans in mocked tests, returns `progression_writes_disabled` by default, fails closed with `idempotency_not_ready` if progression writes are toggled on prematurely, and rejects missing or ambiguous XP/credits paths.
