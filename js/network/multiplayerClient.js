...
(function registerMultiplayerClient(global) {
  ...
  function normalizeBot(bot, fallbackId = "") {
    ...
    return {
      id,
      botType: String(bot.botType || ""),
      type: String(bot.type || "Dev Bot"),
      name: String(bot.name || bot.type || "Dev Bot"),
      displayName: String(bot.displayName || bot.name || bot.type || "Dev Bot"),
      faction: String(bot.faction || ""),
      image: String(bot.image || bot.imageSrc || bot.imagePath || ""),
      threat: String(bot.threat || ""),
      ...
      damagePerHit: Number.isFinite(Number(bot.damagePerHit)) ? Number(bot.damagePerHit) : 0,
      attackCooldownMs: Number.isFinite(Number(bot.attackCooldownMs)) ? Number(bot.attackCooldownMs) : 0,
      visualScale: Number.isFinite(Number(bot.visualScale)) ? Number(bot.visualScale) : 1,
  ...
  function normalizeStagingBounty(bounty) {
    ...
    return {
      ...
      contractType: String(bounty.contractType || ""),
      targetType: String(bounty.targetType || ""),
      targetFaction: String(bounty.targetFaction || ""),
      target: String(bounty.target || ""),
      targetBotType: String(bounty.targetBotType || ""),
      targetBotLabel: String(bounty.targetBotLabel || ""),
      difficulty: String(bounty.difficulty || ""),
      ...
      lupenShardsReward: Number.isFinite(Number(bounty.lupenShardsReward)) ? Number(bounty.lupenShardsReward) : 0,
      icon: String(bounty.icon || ""),
      fallbackIcon: String(bounty.fallbackIcon || ""),
      timed: bounty.timed === true,
      timeLimitSeconds: Number.isFinite(Number(bounty.timeLimitSeconds)) ? Number(bounty.timeLimitSeconds) : 0,
      expiresAt: Number.isFinite(Number(bounty.expiresAt)) ? Number(bounty.expiresAt) : 0,
  ...
  function normalizeStagingXpResult(message) {
    ...
    return {
      ...
      debugReason: String(message.debugReason || ""),
      botId: String(message.botId || ""),
      botType: String(message.botType || ""),
      botName: String(message.botName || "Staging Bot"),
      displayName: String(message.displayName || message.botName || "Staging Bot"),
      image: String(message.image || ""),
      threat: String(message.threat || ""),
      damagePerHit: Number.isFinite(Number(message.damagePerHit)) ? Number(message.damagePerHit) : 0,
      attackCooldownMs: Number.isFinite(Number(message.attackCooldownMs)) ? Number(message.attackCooldownMs) : 0,
      rewardPreviewId: String(message.rewardPreviewId || ""),
  ...
  function normalizeStagingReturnFire(message) {
    ...
    return {
      ...
      attackerBotType: String(message.attackerBotType || ""),
      attackerImage: String(message.attackerImage || ""),
      attackerThreat: String(message.attackerThreat || ""),
      currentNode: String(message.currentNode || ""),
      damage: Number.isFinite(Number(message.damage)) ? Math.max(0, Number(message.damage)) : 0,
      damagePerHit: Number.isFinite(Number(message.damagePerHit ?? message.botDamage ?? message.damage)) ? Math.max(0, Number(message.damagePerHit ?? message.botDamage ?? message.damage)) : 0,
      baseDamagePerHit: Number.isFinite(Number(message.baseDamagePerHit ?? message.damagePerHit ?? message.botDamage ?? message.damage)) ? Math.max(0, Number(message.baseDamagePerHit ?? message.damagePerHit ?? message.botDamage ?? message.damage)) : 0,
      fireRole: String(message.fireRole || "primary"),
      damageMultiplier: Number.isFinite(Number(message.damageMultiplier)) ? Math.max(0, Number(message.damageMultiplier)) : 1,
  ...
  function normalizeStagingResourceEvent(message) {
    ...
    return {
      ...
      volleyWeaponCount: Number.isFinite(Number(message.volleyWeaponCount)) ? Math.max(0, Number(message.volleyWeaponCount)) : 0,
      volleyWeaponKeys: Array.isArray(message.volleyWeaponKeys) ? message.volleyWeaponKeys.map((key) => String(key || "")).filter(Boolean) : [],
  ...
  function bindRoomEvents(activeRoom) {
    ...
    activeRoom.onMessage("bot:disabled", (message) => {
      ...
      connection.lastBotEvent = {
        ...
        damagePerHit: Number.isFinite(Number(message?.damagePerHit)) ? Number(message.damagePerHit) : 0,
        attackCooldownMs: Number.isFinite(Number(message?.attackCooldownMs)) ? Number(message.attackCooldownMs) : 0,
        visualScale: Number.isFinite(Number(message?.visualScale)) ? Number(message.visualScale) : 1,
        currentNode: String(message?.currentNode || ""),
        shield: Number.isFinite(Number(message?.shield)) ? Number(message.shield) : 0,
        hull: Number.isFinite(Number(message?.hull)) ? Number(message.hull) : 0,
    ...
    activeRoom.onMessage("bot:respawned", (message) => {
      connection.lastBotEvent = {
        ...
        botId: String(message?.botId || ""),
        botType: String(message?.botType || ""),
        botName: String(message?.botName || message?.displayName || "Staging Bot"),
        displayName: String(message?.displayName || message?.botName || "Staging Bot"),
        image: String(message?.image || ""),
        threat: String(message?.threat || ""),
        damagePerHit: Number.isFinite(Number(message?.damagePerHit)) ? Number(message.damagePerHit) : 0,
    ...
    activeRoom.onMessage("staging:shot", (message) => {
      ...
      connection.lastCombatVisualEvent = {
        ...
        targetBotId,
        botType: connection.lastShotEvent.botType,
        botName: connection.lastShotEvent.botName,
        displayName: connection.lastShotEvent.displayName,
        image: connection.lastShotEvent.image,
        threat: connection.lastShotEvent.threat,
        ...
        volleyWeaponCount: connection.lastShotEvent.volleyWeaponCount,
        volleyWeaponKeys: connection.lastShotEvent.volleyWeaponKeys,
